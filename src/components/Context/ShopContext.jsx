...
const ShopContextProvider = (props) => {
    ...
    useEffect(()=>{
        fetch('https://onlineshop-backend-63tt.onrender.com/allproducts')
        .then((resp)=>resp.json()).then((data)=>setAllProducts(data))

        if(localStorage.getItem('auth-token')){
            fetch('https://onlineshop-backend-63tt.onrender.com/getcart',{
                method:'POST',
                headers:{
    ...
    const addToCart= (itemId)=>{
        ...
        if(localStorage.getItem('auth-token')){
            fetch ('https://onlineshop-backend-63tt.onrender.com/addtocart',{
                method: 'POST',
                headers:{
                    Accept: 'application/form-data',
                    'auth-token': `${localStorage.getItem('auth-token')}`,
    ...
    const removeFromCart=(itemId)=>{
        setCartItems((prev)=>({...prev,[itemId]:prev[itemId]-1 }));
        if(localStorage.getItem('auth-token')){
            fetch ('https://onlineshop-backend-63tt.onrender.com/removefromcart',{
                method: 'POST',
                headers:{
                    Accept: 'application/form-data',
                    'auth-token': `${localStorage.getItem('auth-token')}`,
